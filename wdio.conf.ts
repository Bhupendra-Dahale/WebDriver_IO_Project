...
export const config: Options.Testrunner = {
  ...
  suites: {
    ...
    OrangeHRM: [
      [
        './test/features/OrangeHRM/*.feature'
      ]
    ]
  },
  // Patterns to exclude.
  ...
  capabilities: [
    {
      ...
      "goog:chromeOptions": {
        ...
        args: headless === "Y" ? [ "--headless", "--disable-dev-shm-usage", "--no-sandbox", "--window-size=1920,1080"] : []
      },
  ...
  logLevel: debug = "info" ,//debug!.toUpperCase() === "Y" ? 'info' : 'error'
  //
  ...
  // 'https://admin:admin@the-internet.herokuapp.com/',   /*Basic Authentication*/
  //
  ...
  cucumberOpts: {
    ...
    require: [`${process.cwd()}/test/features/step-definitions/*.ts`], //This is an Absolute path of the feature file
    // <boolean> show full backtrace for errors                            //`./test/features/step-definitions/*.ts` is relative path
    backtrace: false,
  ...
  //   browser.options["OpenHrmUrl"] = config.OpenHrmUrl
  // },
  /**
  ...
  beforeScenario: function (world, context) {
    ...
    if(!browser.options.testid) throw Error(`>>> TestID is not present : ${world.pickle.name}`)
  },
  /**
   *
  ...
  //   //[need to add @ts.nockeck to avoid type check]
  //   //Add more Env  -  but addEnvironment is not working
  //   allure.addEnvironment("Environment:", browser.config.environment)
  //   allure.addEnvironment("UAT:", "QA5")
  // },
